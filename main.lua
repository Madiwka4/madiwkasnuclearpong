...
function nettest(dt)
    ...
    if data then
        ...
        if (ball[1].dx > 0 and ball[1].x >= 40) or (ball[1].dx <= 0 and ball[1].x > VIRTUAL_WIDTH - 40) then 
            die = tonumber(p[2])
            lastSentKeyClient, 
            ball[1].dy, 
            player2.y,
...
function clienttest(dt) 
    ...
    if data then
        ...
        if p[15] then 
            ...
            if (ball[i].dx <= 0 and ball[i].x < VIRTUAL_WIDTH - 40) or (ball[i].dx > 0 and ball[i].x < 40) then 
